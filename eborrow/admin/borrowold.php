          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                   // $sql = "SELECT *,location.loc, borrow.id, employees.employee_id, borrow.date_borrow, employees.firstname, employees.lastname , department.depname, borrow.date_issued, borrow.due_date, borrow.status, category.name, borrow.item FROM borrow INNER JOIN employees ON borrow.employee_id = employees.id INNER JOIN department ON employees.department_id = department.id INNER JOIN category ON borrow.item_id = category.name INNER JOIN location on borrow.location=location.id LEFT JOIN tbl_inventory ON borrow.item_id = tbl_inventory.id WHERE borrow.status =2 or borrow.status =4 or borrow.status =5 ";
					$sql = "SELECT *, borrow.id, borrow.employee_id, borrow.item_id, borrow.date_borrow,
  borrow.due_date, employees.firstname, employees.lastname, borrow.item,
  department.depname, borrow.location, borrow.status
FROM borrow INNER JOIN
  employees ON borrow.employee_id = employees.id INNER JOIN
  department ON employees.department_id = department.id INNER JOIN
  location ON borrow.location = location.id
WHERE (borrow.status = 2) OR
  (borrow.status = 4) OR
  (borrow.status = 5)";
                    ...
                    while($row = $query->fetch_assoc()){
                      ...
                      echo "
                        <tr>
						  ...
						  <td>".$status."</td>
						      <td>
							<button class='btn btn-info btn-sm edit btn-flat' data-id='".$vid."'><i class='fa fa-undo'></i> Returned </button>
