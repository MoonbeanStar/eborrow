...
while($row=mysqli_fetch_array($result1, MYSQLI_ASSOC))
{
	 ...
	 $itemben = $row['itemcode'];
                         ...
                         // $sql = "SELECT * FROM items where status = 0 order by depname asc";
						  $sql ="SELECT tbl_inventory.itemcode
FROM borrow LEFT JOIN tbl_inventory ON borrow.item_id = tbl_inventory.item
WHERE tbl_inventory.status='ACTIVE' AND borrow.id='$vid'";
